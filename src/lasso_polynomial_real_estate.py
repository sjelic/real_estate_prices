...
gs = GridSearchCV(
        estimator=make_pipeline(StandardScaler(), linear_model.Lasso(max_iter=5000)),
        param_grid={'lasso__alpha': list(np.arange(1,12,0.2)) },
        scoring=scoring,
        refit='r2',
...
reg_coefs.to_excel('../results/lasso_polynomial_reg_coefs.xlsx', index=False)
gs_crossvalidation_results.to_excel('../results/lasso_polynomial_results.xlsx', index=False)

print(
    f"""
    EVALUATION METRICS:
        Best alpha: {gs_all_metric_results.best_params_['lasso__alpha']},
...
metrics = {
    'best_alpha': gs_all_metric_results.best_params_['lasso__alpha'],
...
with open('../results/lasso_polynomial_metrics.json', 'w') as f:
    json.dump(metrics, f)