...
gs = GridSearchCV(
        estimator=make_pipeline(StandardScaler(), linear_model.Lasso(max_iter=10000)),
...
reg_coefs.to_excel('../results/lasso_reg_coefs.xlsx', index=False)
gs_crossvalidation_results.to_excel('../results/lasso_cv_results.xlsx', index=False)
...
with open('../results/lasso_metrics.json', 'w') as f:
    json.dump(metrics, f)