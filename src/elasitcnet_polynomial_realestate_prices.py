...
numerical_features_pot = []
for i in range(len(numerical_features)):
    for j in range(i,len(numerical_features)):
        numerical_features_pot.append({'name': f"{numerical_features[i]['name']}_times_{numerical_features[j]['name']}", 'description': f"Product {numerical_features[i]['name']} times {numerical_features[j]['name']}", 'group': numerical_features[i]['group'], 'type':numerical_features[i]['type'] })
    
numerical_features_pot_names = [x['name'] for x in numerical_features_pot]

X_new = pd.DataFrame(columns=numerical_features_names + numerical_features_pot_names)
X_new[numerical_features_names] = X[numerical_features_names]

for i in range(len(numerical_features_names)):
    for j in range(i,len(numerical_features_names)):
        X_new[f"{numerical_features_names[i]}_times_{numerical_features_names[j]}"] = X[numerical_features_names[i]]*X[numerical_features_names[j]]
...
gs = GridSearchCV(
        estimator=make_pipeline(StandardScaler(), linear_model.ElasticNet(max_iter=10000)),
...
reg_coefs.to_excel('../results/elasticnet_polynomial_reg_coefs.xlsx', index=False)
gs_crossvalidation_results.to_excel('../results/elasticnet_polynomial_cv_results.xlsx', index=False)


print(
    f"""
    EVALUATION METRICS:
        Best alpha: {gs_all_metric_results.best_params_['elasticnet__alpha']},
        Best l1_ratio: {gs_all_metric_results.best_params_['elasticnet__l1_ratio']},
...
metrics = {
    'best_alpha': gs_all_metric_results.best_params_['elasticnet__alpha'],
    'best_l1_ratio': gs_all_metric_results.best_params_['elasticnet__l1_ratio'],
...
with open('../results/elasticnet_polynomial_metrics.json', 'w') as f:
    json.dump(metrics, f)