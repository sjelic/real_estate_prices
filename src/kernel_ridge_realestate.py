...
gs = GridSearchCV(
        estimator=make_pipeline(StandardScaler(), kernel_ridge.KernelRidge(kernel=pairwise.polynomial_kernel, degree = 2)),
        param_grid={'kernelridge__alpha': list(np.arange(1,5,0.2)) },
        scoring=scoring,
        refit='r2',
...
reg_coefs.to_excel('./clanci/realestate_prices/results/kernel_ridge_reg_coefs.xlsx', index=False)
gs_crossvalidation_results.to_excel('./clanci/realestate_prices/results/kernel_ridge_cv_results.xlsx', index=False)
...
with open('./clanci/realestate_prices/results/kernel_ridge_metrics.json', 'w') as f:
